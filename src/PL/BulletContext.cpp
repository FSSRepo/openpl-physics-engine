...
btTypedConstraint* ConstraintQueue::enqueue(){
...
btRaycastVehicle* VehicleQueue::enqueue(btDefaultVehicleRaycaster* dvrc,btRaycastVehicle::btVehicleTuning tuning){
	if(!chassis){
		return NULL;
	}
...
btCollisionShape* CSQueue::enqueue(Plenum type,int* result,DebugLogger* debug){
	*result = PL_NO_ERROR;
	if(type == PL_BOX_SHAPE){
		...
		mesh.m_numVertices = vertexCount / 3;
		mesh.m_vertexBase = (const unsigned char*)temp_vertex;
		mesh.m_vertexStride = 3;
		btTriangleIndexVertexArray* container = new btTriangleIndexVertexArray();
		container->addIndexedMesh(mesh,PHY_SHORT);
		return new btBvhTriangleMeshShape(container,true,true);
	}else if(type == PL_CONVEX_HULL_SHAPE){
		btConvexHullShape* shape = new btConvexHullShape(temp_vertex,vertexCount / 3,3);
		free(temp_vertex); // delete because bt convex hull creates a internal copy
		temp_vertex = NULL;
